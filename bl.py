...
def main():
    ...
    seperate_plate_from_body = False
    ...
    def column_offset(column: int) -> list:
        return col_offsets.get(column, [0,0,0])
    ...
    th_layout = [[ [ -4, -35, 52],  (-56.3, -43.3, -23.5)],
                 ...
                 [ [  6, -34, 40],  (  -51,   -25,   -12)],
                 [ [ -6, -34, 48],  (  -29,   -40,   -13)],
                 [ [ 10, -23, 10],  (  -32,   -15,    -2), '1.5u'],
                 [ [ 10, -23, 10],  (  -12,   -16,     3), '1.5u']]
    
    def th_get_rot(key):
        entry = th_layout[key]
        return entry['rot'] if isinstance(entry, dict) else entry[0]
        
    def th_get_pos(key):
        entry = th_layout[key]
        return entry['pos'] if isinstance(entry, dict) else entry[1]
        
    def th_get_size(key):
        entry = th_layout[key]
        if isinstance(entry, dict):
            return entry.get('size', '1u')
        return entry[2] if len(entry) > 2 else '1u'
    ...
    plate_border_0 = 1                          # distance of 1st key plate boarder
    ...
    pcb_mount_holes     = False
    pcb_hole_diameter   = 2.2
    pcb_hole_depth      = 12
    pcb_hole_offset     = [(0, 7.5), (0, -7.5)]
    ...
    for collection in ["AXIS", "KEYCAP_PROJECTION_OUTER", "KEYCAP_PROJECTION_INNER", "SWITCH_PROJECTION", "SWITCH_PROJECTION_INNER", "SWITCH_HOLE", "SWITCH_HOLE_LOWER", "SWITCH_SUPPORT", "AMEOBA_CUT", "PCB_MOUNT_HOLES"]:
    ...
    for size in [1, 1.5]:
        for shape in [['switch_projection', (0, 0, 2*mount_thickness-1), (mount_width, mount_height*size,  4*mount_thickness)],
                      ...
                      ['keycap_projection_outer', (0, 0, mount_thickness + 5 + 2), (19, 19*size, 10)],
                      ['keycap_projection_inner', (0, 0, mount_thickness + 5 + 2 - 2), (19+2, 19*size+2, 10)],
                      ['switch_hole', (0, 0, 0), (keyswitch_width, keyswitch_height, 3*mount_thickness)],
                      ['switch_hole_lower', (0, 0, mount_thickness-2), (5, keyswitch_height + 1, 2)],
            ...
            if shape[0] in ['keycap_projection_outer']:
                bpy.ops.object.mode_set(mode = 'EDIT')
                bpy.ops.mesh.select_all(action='DESELECT')
                grid_mesh = bmesh.from_edit_mesh(bpy.context.object.data)
                ...
                for edge in [1, 3, 6, 9]:
                    grid_mesh.edges[edge].select = True
    ...
    for column in range(ncols):
        for row in range(nrows):
            if (column in stagger_cols) or (not row == lastrow):
                ...
                if column==wide_pinky_col and wide_pinky:
                ...
                for tool in [['AXIS',                           'key_axis',                          'key_axis'                           ],
                             ...
                             ['SWITCH_HOLE_LOWER',              'switch_hole_lower_1u',              'switch_hole_lower_1.5u'             ],
                    ...
                    if column==wide_pinky_col and wide_pinky:
                        ...
                        if tool[0] not in ['AXIS', 'SWITCH_HOLE_LOWER', 'SWITCH_SUPPORT', 'AMEOBA_CUT']:
                ...
                if (column == thump_origin_col and row == thump_origin_row):
                ...
                # Select all tools
                for tool in ['AXIS', 'KEYCAP_PROJECTION_OUTER', 'KEYCAP_PROJECTION_INNER', 'SWITCH_PROJECTION', 'SWITCH_PROJECTION_INNER', 'SWITCH_HOLE', 'SWITCH_HOLE_LOWER', 'SWITCH_SUPPORT', 'AMEOBA_CUT']:
    ...
    for key in range(len(th_layout)):
        ...
        # Apply rotation    
        bpy.ops.transform.rotate(value=radians(th_get_rot(key)[0]), orient_axis='X', center_override=(0.0, 0.0, 0.0))
        bpy.ops.transform.rotate(value=radians(th_get_rot(key)[1]), orient_axis='Y', center_override=(0.0, 0.0, 0.0))
        bpy.ops.transform.rotate(value=radians(th_get_rot(key)[2]), orient_axis='Z', center_override=(0.0, 0.0, 0.0))
        ...
        bpy.ops.transform.translate(value=th_get_pos(key), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False)
    ...
    for shape in ['keycap_projection_outer_', 'keycap_projection_inner_', 'switch_projection_', 'switch_projection_inner_', 'switch_hole_', 'switch_hole_lower_', 'switch_support_', 'ameoba_cut_']:
        for size in [1, 1.5]:
    ...
    for finger_plate in ["top", "bottom"]:
        ...
        bpy.ops.mesh.primitive_grid_add(x_subdivisions=2*lastrow, y_subdivisions=2*lastcol, size=1, rotation=(0, 0, -pi/2))
        bpy.ops.transform.resize(value=(2*lastcol, 2*lastrow, 1))
        bpy.ops.object.transform_apply(location=True, rotation=True, scale=True)
        ...
        for column in range(ncols):
            for row in range(nrows):
                if (column in stagger_cols) or (not row == lastrow):
                    ...
                    face_is_a_key.append(row * 2 + column* 2*(2*lastrow))            
                    ...
                    switch_size = 1
                    if column==wide_pinky_col and wide_pinky: switch_size = 1.5
        ...
        for column in range(lastcol):
            for row in range(2*lastrow):
                grid_mesh.faces[(2*lastrow)*(1+2*column)+row].select = True
        ...
        for row in range(lastrow):
            for column in range(2*lastcol):
                grid_mesh.faces[2*row+1 + column*(2*lastrow)].select = True
        ...
        for side in [['finger_TOP',          [0, nrows*(lastcol)*4 + nrows*2]],
                     ['finger_LEFT',         [0, nrows*2-3]],
                     ['finger_RIGHT',        [nrows*(lastcol)*4 + nrows*2, nrows*ncols*4-3]],
                     ['finger_BOTTOM',       [nrows*14-1, nrows*ncols*4-3]],
                     ['finger_corner_BL',    [nrows*2-3]],
                     ['finger_corner_TL',    [0]],
                     ['finger_corner_TR',    [nrows*(lastcol)*4 + nrows*2]],
    ...
    for thumb_plate in ["top", "bottom"]:
        ...
        for thumb in range(len(faces_to_use)):
            ...
            switch_size = 1
            if th_get_size(thumb) == '1.5u' : switch_size = 1.5
            if thumb_plate == "top":
            ...
            if thumb_plate == "bottom":
                if amoeba_style != "none": switch_size = 1
        ...
        for correction in [[ 0,  4,  8],
            ...
            for vertex in correction:
                grid_mesh.verts[vertex].select = True
            if thumb_plate == "bottom" and correction == [ 9, 10, 13, 14, 17]: print(zzz)
            bpy.ops.mesh.edge_face_add()
            bpy.ops.mesh.quads_convert_to_tris(quad_method='BEAUTY', ngon_method='BEAUTY')
            bpy.ops.mesh.select_all(action='DESELECT')
    ...
    for column in range(ncols):
        #Isolate edge
        bpy.ops.mesh.select_all(action='SELECT')    
        bpy.ops.object.vertex_group_set_active(group='switch - ' + str(column) + ', 0')
        bpy.ops.object.vertex_group_deselect()
        bpy.ops.object.vertex_group_set_active(group='finger_row_gap_0')
        bpy.ops.object.vertex_group_select()
        bpy.ops.mesh.select_all(action='INVERT')
        
        #Extrude Border
        bpy.ops.mesh.offset_edges(geometry_mode='extrude', width=plate_border_0, follow_face=True, caches_valid=False)
    ...
    for column in range(lastcol):
        ...
        for vertex_group in ['finger_col_gap_' + str(column)]:
            bpy.ops.object.vertex_group_set_active(group=vertex_group)
            bpy.ops.object.vertex_group_select()
        for vertex_group in ['finger_row_gap_0', 'finger_row_gap_1', 'finger_row_gap_2', 'finger_row_gap_3', 'finger_BOTTOM', 'BRIDGE_LEFT', 'BRIDGE_MID', 'BRIDGE_RIGHT']:
            bpy.ops.object.vertex_group_set_active(group=vertex_group)
            bpy.ops.object.vertex_group_deselect()
        bpy.ops.mesh.edge_face_add()
        bpy.ops.mesh.select_all(action='DESELECT')
    ...
    for column in range(ncols):
        #Isolate edge
        bpy.ops.mesh.select_all(action='SELECT')
        if column in stagger_cols:
            bpy.ops.object.vertex_group_set_active(group='switch - ' + str(column) + ', ' + str(lastrow))
            bpy.ops.object.vertex_group_deselect()
            bpy.ops.object.vertex_group_set_active(group='finger_row_gap_' + str(lastrow - 1))
            bpy.ops.object.vertex_group_select()
        else:
            bpy.ops.object.vertex_group_set_active(group='switch - ' + str(column) + ', ' + str(cornerrow))
            bpy.ops.object.vertex_group_deselect()
            bpy.ops.object.vertex_group_set_active(group='finger_row_gap_' + str(cornerrow -1))
            bpy.ops.object.vertex_group_select()      
        bpy.ops.mesh.select_all(action='INVERT')
    ...
    for column in range(lastcol):
        ...
        if column in [stagger_cols[0] - 1, stagger_cols[-1]]:
            bpy.ops.object.vertex_group_set_active(group='switch - ' + str(stagger_cols[0]) + ', ' + str(cornerrow))
            bpy.ops.object.vertex_group_deselect()
    ...
    bpy.ops.object.vertex_group_set_active(group="RAISE_0")
    bpy.ops.object.vertex_group_select()
    for vertex_group in ['key_finger']:
        bpy.ops.object.vertex_group_set_active(group=vertex_group)
        bpy.ops.object.vertex_group_remove_from()
    bpy.ops.mesh.select_all(action='DESELECT')
    ...
    bpy.ops.object.vertex_group_set_active(group="key_finger")
    bpy.ops.object.vertex_group_select()
    for vertex_group in ['finger_TOP', 'finger_LEFT', 'finger_RIGHT', 'finger_BOTTOM', 'finger_corner_BL', 'finger_corner_TL', 'finger_corner_TR', 'finger_corner_BR', 'BRIDGE_LEFT', 'BRIDGE_MID', 'BRIDGE_RIGHT', 'BRIDGE_LEFT_RING_0', 'BRIDGE_RIGHT_RING_0']:
        bpy.ops.object.vertex_group_set_active(group=vertex_group)
        bpy.ops.object.vertex_group_remove_from()
    bpy.ops.mesh.select_all(action='DESELECT')


    #Correct Corner Groups
    for vertex_group in ['finger_corner_TL', 'finger_corner_BL', 'finger_corner_TR', 'finger_corner_BR']:
        bpy.ops.object.vertex_group_set_active(group=vertex_group)
        bpy.ops.object.vertex_group_select()
    ...
    bpy.ops.object.vertex_group_set_active(group="finger_row_gap_0")
    bpy.ops.object.vertex_group_select()
    bpy.ops.object.vertex_group_set_active(group="finger_row_gap_2")
    bpy.ops.object.vertex_group_select()
    ...
    # create bottom plate outline

    if seperate_plate_from_body:
        bpy.ops.object.vertex_group_set_active(group="RAISE_0")
        bpy.ops.object.vertex_group_select()

        bpy.ops.mesh.offset_edges(geometry_mode='offset', width=5, angle=0, follow_face=True, caches_valid=False)
        bpy.ops.object.vertex_group_set_active(group='RAISE_0')
        bpy.ops.object.vertex_group_assign()
        ...
        bpy.ops.object.vertex_group_set_active(group="RAISE_0")
        bpy.ops.object.vertex_group_select()
        bpy.ops.mesh.offset_edges(geometry_mode='offset', width=body_thickness*sqrt(2), angle=-0.785398, follow_face=True, caches_valid=False)
        bpy.ops.object.vertex_group_set_active(group='RAISE_0')
        bpy.ops.object.vertex_group_assign()
        bpy.ops.mesh.separate(type="LOOSE")
    ...
    for column in range(lastcol):
        ...
        for vertex_group in ['finger_col_gap_' + str(column)]:
            bpy.ops.object.vertex_group_set_active(group=vertex_group)
            bpy.ops.object.vertex_group_select()
        for vertex_group in ['key_finger', 'finger_row_gap_0', 'finger_row_gap_1', 'finger_row_gap_2', 'finger_row_gap_3', 'finger_BOTTOM', 'BRIDGE_LEFT', 'BRIDGE_MID', 'BRIDGE_RIGHT']:
            bpy.ops.object.vertex_group_set_active(group=vertex_group)
            bpy.ops.object.vertex_group_deselect()
        bpy.ops.mesh.edge_face_add()
        bpy.ops.mesh.select_all(action='DESELECT')
    ...
    if seperate_plate_from_body:
        ...
        bpy.ops.object.vertex_group_set_active(group="RAISE_1")
        bpy.ops.object.vertex_group_select()
        bpy.ops.mesh.offset_edges(geometry_mode='offset', width=-body_thickness, angle=0, follow_face=True, caches_valid=False)
        bpy.ops.object.vertex_group_set_active(group='RAISE_1')
        bpy.ops.object.vertex_group_assign()
        bpy.ops.mesh.separate(type="LOOSE")
    else:
        bpy.ops.object.vertex_group_set_active(group="RAISE_1")
        bpy.ops.object.vertex_group_select()
        bpy.ops.mesh.offset_edges(geometry_mode='offset', width=body_thickness*sqrt(2), angle=-2.35619, follow_face=True, caches_valid=False)
        bpy.ops.object.vertex_group_set_active(group='RAISE_1')
        bpy.ops.object.vertex_group_assign()
        bpy.ops.mesh.separate(type="LOOSE")
    ...
    ####
    #    Manual fix of "corners" on keycaps
    ####
    ...
    bpy.ops.object.vertex_group_deselect()
    bpy.ops.object.vertex_group_assign_new()
    bpy.data.objects['finger_plate_top'].vertex_groups['Group'].name = 'TEMP_CORNER_SQUARE'
    
    bpy.ops.object.mode_set(mode = 'OBJECT')
    bpy.ops.object.modifier_add(type='SHRINKWRAP')
    bpy.context.object.modifiers["Shrinkwrap"].wrap_method = 'NEAREST_VERTEX'
    bpy.context.object.modifiers["Shrinkwrap"].target = bpy.data.objects["keycap_projection_outer - " + str(stagger_cols[-1]) + ', ' + str(lastrow)]
    ...
    bpy.ops.object.modifier_add(type='SHRINKWRAP')
    bpy.context.object.modifiers["Shrinkwrap"].wrap_method = 'NEAREST_VERTEX'
    bpy.context.object.modifiers["Shrinkwrap"].target = bpy.data.objects["keycap_projection_outer - " + str(stagger_cols[0]) + ', ' + str(lastrow)]
    bpy.context.object.modifiers["Shrinkwrap"].vertex_group = "TEMP_CORNER_SQUARE"
    bpy.context.object.modifiers["Shrinkwrap"].offset = 0.01
    bpy.ops.object.modifier_apply(modifier="Shrinkwrap")
    bpy.ops.object.mode_set(mode = 'EDIT')
    ...
    for column in range(lastcol):
        ...
        for vertex_group in ['finger_col_gap_' + str(column)]:
            bpy.ops.object.vertex_group_set_active(group=vertex_group)
            bpy.ops.object.vertex_group_select()
        for vertex_group in ['key_finger', 'RAISE_0', 'finger_row_gap_0', 'finger_row_gap_1', 'finger_row_gap_2', 'finger_row_gap_3', 'finger_BOTTOM', 'BRIDGE_LEFT', 'BRIDGE_MID', 'BRIDGE_RIGHT']:
            bpy.ops.object.vertex_group_set_active(group=vertex_group)
            bpy.ops.object.vertex_group_deselect()
        bpy.ops.mesh.edge_face_add()
        bpy.ops.mesh.select_all(action='DESELECT')
    ...
    bpy.ops.object.vertex_group_set_active(group='TEMP')
    bpy.ops.object.vertex_group_assign()
    bpy.ops.object.vertex_group_set_active(group='finger_col_gap_3')
    bpy.ops.object.vertex_group_select()
    bpy.ops.object.vertex_group_set_active(group='finger_col_gap_' + str(ncols-2)')
    bpy.ops.object.vertex_group_select()
    ...
    bpy.ops.object.vertex_group_set_active(group='TEMP')
    bpy.ops.object.vertex_group_deselect()
    bpy.ops.object.vertex_group_set_active(group='switch - 5, 0')
    bpy.ops.object.vertex_group_deselect()
    bpy.ops.mesh.edge_face_add()
    
    bpy.ops.object.vertex_group_set_active(group='finger_col_gap_' + str(ncols-2)')
    bpy.ops.object.vertex_group_deselect()
    bpy.ops.object.vertex_group_set_active(group='switch - 3, 0')
    bpy.ops.object.vertex_group_deselect()
    bpy.ops.object.vertex_group_set_active(group='RAISE_2')
    bpy.ops.object.vertex_group_remove_from()
    bpy.ops.mesh.select_all(action='DESELECT')
    ...
    for column in range(ncols):
        ...
        if column <4:
            if column <2:
                ...
                bpy.ops.mesh.offset_edges(geometry_mode='extrude', width=-7 - (1-sin(alpha)/2)*column_offset(column)[1] + (column_offset(column)[2]/2) , depth_mode='angle', angle=1.5708, follow_face=True, caches_valid=False, angle_presets='90°')
                if column == 2: bpy.ops.transform.resize(value=(0.5, 0.5, 0.5), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(True, True, True), mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False, snap=False, snap_elements={'INCREMENT'}, use_snap_project=False, snap_target='CLOSEST', use_snap_self=True, use_snap_edit=True, use_snap_nonedit=True, use_snap_selectable=False)
                if column == 3: bpy.ops.transform.resize(value=(0.95, 0.95, 0.95), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(True, True, True), mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False, snap=False, snap_elements={'INCREMENT'}, use_snap_project=False, snap_target='CLOSEST', use_snap_self=True, use_snap_edit=True, use_snap_nonedit=True, use_snap_selectable=False)
        else:
            bpy.ops.mesh.offset_edges(geometry_mode='extrude', width=-7, depth_mode='angle', angle=1.5708, follow_face=True, caches_valid=False, angle_presets='90°')
            if column == 4: bpy.ops.transform.resize(value=(0.5, 0.5, 0.5), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', constraint_axis=(True, True, True), mirror=True, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False, snap=False, snap_elements={'INCREMENT'}, use_snap_project=False, snap_target='CLOSEST', use_snap_self=True, use_snap_edit=True, use_snap_nonedit=True, use_snap_selectable=False)
        bpy.ops.object.vertex_group_set_active(group='RAISE_2')
        bpy.ops.object.vertex_group_assign()
        bpy.ops.mesh.select_all(action='DESELECT')
    ...
    for vertex_group in ['finger_corner_TL', 'finger_corner_BL', 'finger_corner_TR', 'finger_corner_BR']:
        ...
        for vertex_group in ['finger_TOP', 'finger_LEFT', 'finger_RIGHT', 'finger_BOTTOM', 'BRIDGE_LEFT']:
            bpy.ops.object.vertex_group_set_active(group=vertex_group)
            bpy.ops.object.vertex_group_deselect()
        bpy.ops.mesh.edge_face_add()
        bpy.ops.mesh.select_all(action='DESELECT')
    ...
    bpy.ops.object.vertex_group_set_active(group="RAISE_2")
    bpy.ops.object.vertex_group_select()
    bpy.ops.mesh.offset_edges(geometry_mode='offset', width=-2*sqrt(2), angle=0.7853982, follow_face=True, edge_rail=True, caches_valid=False)
    bpy.ops.object.vertex_group_set_active(group='RAISE_2')
    bpy.ops.object.vertex_group_assign()
    bpy.ops.mesh.separate(type="LOOSE")
    ...
    ###
    #  Add corner Verteces to respective sides
    ###
    for side in ["finger_TOP", "finger_LEFT", "finger_RIGHT"]:
        bpy.ops.object.vertex_group_set_active(group=side)
        bpy.ops.object.vertex_group_select()
        bpy.ops.mesh.select_more()
        bpy.ops.object.vertex_group_set_active(group="RAISE_1")
    ...
    if seperate_plate_from_body:
        ...
        bpy.context.selected_objects[0].name = "seperated_finger_plate"
        
        
        bpy.ops.object.select_all(action='DESELECT')
        bpy.context.view_layer.objects.active = bpy.data.objects['finger_plate_top.002']
        bpy.data.objects["finger_plate_top.002"].select_set(True)
        bpy.context.selected_objects[0].name = "finger_plate_bottom"
        
        bpy.data.objects["finger_plate_top.003"].select_set(True)
        bpy.ops.object.join()
    ...
    if seperate_plate_from_body:
        ...
        bpy.ops.mesh.select_all(action='DESELECT')
        bpy.ops.mesh.select_non_manifold()
        bpy.ops.mesh.offset_edges(geometry_mode='offset', width=0.5, follow_face=True, caches_valid=False)
        bpy.ops.mesh.separate(type="LOOSE")
        
        bpy.ops.object.mode_set(mode = 'OBJECT')
        bpy.context.selected_objects[0].name = "seperated_finger_plate_top"

        bpy.ops.object.select_all(action='DESELECT')
        bpy.context.view_layer.objects.active = bpy.data.objects['finger_plate_top.001']
        bpy.data.objects["finger_plate_top.001"].select_set(True)
        bpy.context.selected_objects[0].name = "finger_plate_top"
        ...
        bpy.ops.object.mode_set(mode = 'OBJECT')
        bpy.context.selected_objects[0].name = "seperated_finger_plate_bottom"

        bpy.ops.object.select_all(action='DESELECT')
        bpy.context.view_layer.objects.active = bpy.data.objects['finger_plate_bottom.001']
        bpy.data.objects["finger_plate_bottom.001"].select_set(True)
        bpy.context.selected_objects[0].name = "finger_plate_bottom"
        ...
        bpy.ops.object.select_all(action='DESELECT')
        bpy.context.view_layer.objects.active = bpy.data.objects['seperated_finger_plate_top']
        bpy.data.objects["seperated_finger_plate_top"].select_set(True)
        
        bpy.ops.object.mode_set(mode = 'EDIT')
        bpy.ops.mesh.select_non_manifold()
        
        bpy.ops.mesh.offset_edges(geometry_mode='offset', width=-5, depth=-2, depth_mode='depth', follow_face=True, caches_valid=False)


   
        bpy.ops.mesh.offset_edges(geometry_mode='extrude', width=-body_thickness, angle=1.5708, follow_face=True, caches_valid=False, angle_presets='90°')


        bpy.ops.mesh.offset_edges(geometry_mode='extrude', width=-0.05, angle=1.5708, follow_face=True, caches_valid=False, angle_presets='90°')
    ...
    for bridge in ['BRIDGE_MID', 'BRIDGE_LEFT']:
    ...
    for bridge in ['BRIDGE_LEFT']:
        bpy.ops.object.vertex_group_set_active(group=bridge)
        bpy.ops.object.vertex_group_select()

        if seperate_plate_from_body:
            for ring in ['RAISE_1']:
                bpy.ops.object.vertex_group_set_active(group=ring)
                bpy.ops.object.vertex_group_deselect()
        else:
            for ring in ['RAISE_1', 'RAISE_2']:
                bpy.ops.object.vertex_group_set_active(group=ring)
                bpy.ops.object.vertex_group_deselect()
        bpy.ops.mesh.edge_face_add()
        bpy.ops.mesh.quads_convert_to_tris(ngon_method='BEAUTY')
    ...
    if seperate_plate_from_body:
        ...
        for bridge in ['BRIDGE_LEFT', 'BRIDGE_MID']:
            bpy.ops.object.vertex_group_set_active(group=bridge)
            bpy.ops.object.vertex_group_select()
    ...
    if amoeba_style != 'none':
        ...
        bpy.ops.mesh.select_all(action='INVERT')
        bpy.ops.mesh.select_more()
        bpy.ops.object.vertex_group_set_active(group='switch - 0, ' + str(cornerrow))
        bpy.ops.object.vertex_group_deselect()
        bpy.ops.object.vertex_group_set_active(group="finger_corner_BL")
        bpy.ops.object.vertex_group_select()
        ...
        for vertex in grid_mesh.verts:
            if vertex.select:
                vertex.co = bpy.data.objects["ameoba_cut - 0, " + str(cornerrow)].data.vertices[1].co
                
                
        bpy.context.scene.cursor.location = bpy.data.objects["axis - 0, " + str(cornerrow)].location
        bpy.context.scene.cursor.rotation_euler =  bpy.data.objects["axis - 0, " + str(cornerrow)].rotation_euler
    ...
    need to add corners to respective edges
    ...
    for build_edge in [ ['finger_TOP',    [[wall_thickness, -1],
                        ...
                        ['finger_BOTTOM', [[wall_thickness, -1],
                                           [wall_xy_offset + wall_thickness, wall_z_offset], 
                                           [wall_xy_offset + wall_thickness + 1.5, wall_z_offset -1.5 - wall_thickness]]],
        ...
        for ring_num in range(0, 3):
            ...
            if build_edge[0]=="thumb_RIGHT":
                bpy.ops.object.vertex_group_set_active(group="switch - thumb - " + str(len(th_layout)-1))
                bpy.ops.object.vertex_group_deselect()
                bpy.ops.object.vertex_group_set_active(group=build_edge[0])
                #print(xyz)
            ...
            bpy.ops.object.vertex_group_assign()
            for group in ['key_finger', 'key_thumb', 'finger_LEFT', 'finger_TOP', 'finger_RIGHT', 'finger_BOTTOM', 'thumb_BOTTOM', 'thumb_LEFT', 'thumb_RIGHT', 'RING_0', 'RING_' + str(ring_num), 'RAISE_0', 'RAISE_1', 'RAISE_2']:
                bpy.ops.object.vertex_group_set_active(group=group)
                bpy.ops.object.vertex_group_remove_from()
            bpy.ops.mesh.select_all(action='DESELECT')
    ...
    for corner in [ 'finger_corner_TL', 'finger_corner_TR', 'finger_corner_BR', 'thumb_corner_BL', 'thumb_corner_BR', 'BRIDGE_RIGHT_RING_0']:
    ...
    for i in range(100):
        bpy.ops.mesh.offset_edges(geometry_mode='move', width=1, depth_mode='depth', follow_face=True, caches_valid=False)
        bpy.ops.mesh.remove_doubles(threshold=1)
    '''
    ...
    bpy.ops.object.vertex_group_set_active(group='finger_LEFT')
    bpy.ops.object.vertex_group_remove_from()
    bpy.ops.object.vertex_group_set_active(group='thumb_LEFT')
    bpy.ops.object.vertex_group_remove_from()
    bpy.ops.mesh.select_all(action='DESELECT')
    ...
    for build_edge in [ ['finger_TOP',    [[wall_thickness, -1],
        ...
        for ring_num in range(0, 3):
            bpy.ops.object.vertex_group_set_active(group=build_edge[0])
            bpy.ops.object.vertex_group_select()
            bpy.ops.object.vertex_group_set_active(group='RING_1')
            bpy.ops.object.vertex_group_deselect()
            ...
            with suppress_stdout():
                #if build_edge[0]=="finger_BOTTOM"and ring_num==1: print(xyz)
                bpy.ops.mesh.offset_edges(width=build_edge[1][ring_num][0], depth=-build_edge[1][ring_num][1], depth_mode='depth', follow_face=True, mirror_modifier=False, edge_rail=False, caches_valid=False)
                #if build_edge[0]=="finger_BOTTOM": print(xyz)
            bpy.ops.object.vertex_group_set_active(group='RING_' + str(ring_num+1))
            bpy.ops.object.vertex_group_assign()
            ...
            for group in ['key_finger', 'key_thumb', 'finger_LEFT', 'finger_TOP', 'finger_RIGHT', 'finger_BOTTOM', 'thumb_BOTTOM', 'thumb_LEFT', 'thumb_RIGHT', 'RING_0', 'RING_' + str(ring_num), 'RAISE_0', 'RAISE_1', 'RAISE_2']:
                ...
                bpy.ops.object.vertex_group_remove_from()
            '''
            for group in ['key_finger', 'key_thumb', 'RING_0', 'RING_' + str(ring_num), 'RAISE_0', 'RAISE_1', 'RAISE_2']:
                bpy.ops.object.vertex_group_set_active(group=group)
                bpy.ops.object.vertex_group_remove_from()
            '''
            bpy.ops.mesh.select_all(action='DESELECT')
    ...
    for corner in [ 'finger_corner_TL', 'finger_corner_TR', 'finger_corner_BR', 'thumb_corner_BL', 'thumb_corner_BR', 'BRIDGE_RIGHT_RING_0']:
        ...
        if not seperate_plate_from_body:
            bpy.ops.object.vertex_group_set_active(group='key_finger')
            bpy.ops.object.vertex_group_select()
            bpy.ops.mesh.select_more()
            bpy.ops.object.vertex_group_set_active(group=corner)
    ...
    '''
    for ring_num in range(1,4):
        #bpy.ops.mesh.select_by_pole_count(pole_count=1, type='EQUAL', extend=False, exclude_nonmanifold=False)
        for counter in range(1,4):
            if counter != ring_num:
                bpy.ops.object.vertex_group_set_active(group="RING_" + str(counter))
                bpy.ops.object.vertex_group_deselect()
        bpy.ops.mesh.edge_face_add()
        bpy.ops.mesh.delete(type='ONLY_FACE')
        bpy.ops.mesh.select_all(action='DESELECT')
    ...
    for projection_type in [['body',       'keycap_projection_outer', mount_thickness + 2, 'all'       ],
        ...
        for thing in bpy.data.collections[projection_type[1].upper()].objects:
            ...
            #if projection_type[1] in ['switch_projection', 'keycap_projection_outer']:
    ...
    if seperate_plate_from_body:
        ...
        bpy.context.object.modifiers["Shrinkwrap"].wrap_method = 'TARGET_PROJECT'
        bpy.context.object.modifiers["Shrinkwrap"].wrap_mode = 'OUTSIDE'
        bpy.context.object.modifiers["Shrinkwrap"].target = bpy.data.objects["body_bottom"]
        bpy.context.object.modifiers["Shrinkwrap"].offset = 1
        bpy.context.object.modifiers["Shrinkwrap"].vertex_group = "RAISE_0"
        bpy.ops.object.modifier_apply(modifier="Shrinkwrap")
    ...
    bpy.ops.mesh.select_all(action='INVERT')
    bpy.ops.mesh.remove_doubles(threshold=1)
    #bpy.ops.mesh.extrude_region_move(TRANSFORM_OT_translate={"value":(0, 0, -bottom_thickness+0.1), "orient_type":'GLOBAL', "orient_matrix":((1, 0, 0), (0, 1, 0), (0, 0, 1)), "orient_matrix_type":'GLOBAL', "constraint_axis":(True, True, True), "mirror":False, "use_proportional_edit":False, "proportional_edit_falloff":'SMOOTH', "proportional_size":1, "use_proportional_connected":False, "use_proportional_projected":False, "snap":False, "snap_elements":{'INCREMENT'}, "use_snap_project":False, "snap_target":'CLOSEST', "use_snap_self":True, "use_snap_edit":True, "use_snap_nonedit":True, "use_snap_selectable":False, "snap_point":(0, 0, 0), "snap_align":False, "snap_normal":(0, 0, 0), "gpencil_strokes":False, "cursor_transform":False, "texture_space":False, "remove_on_cancel":False, "use_duplicated_keyframes":False, "view2d_edge_pan":False, "release_confirm":False, "use_accurate":False, "use_automerge_and_split":False, "translate_origin":False})
    ...
    bpy.context.object.modifiers["Shrinkwrap"].target = bpy.data.objects["body"]
    bpy.context.object.modifiers["Shrinkwrap"].offset = body_thickness
    bpy.context.object.modifiers["Shrinkwrap"].vertex_group = "BRIDGE_LEFT"
    bpy.ops.object.modifier_apply(modifier="Shrinkwrap")




    if not keep_parts_separate:
        bpy.ops.object.select_all(action='DESELECT')
        bpy.data.objects["body"].select_set(True)
        bpy.data.objects["body_bottom"].select_set(True)
        bpy.context.view_layer.objects.active = bpy.data.objects['body']
    ...
    ##########################
    if pcb_mount_holes:
        print("{:.2f}".format(time.time() - start_time), "- Add PCB Mount Holes")
        bpy.ops.wm.redraw_timer(type='DRAW_WIN_SWAP', iterations = 1)
        ...
        for axis_obj in bpy.data.collections["AXIS"].objects:
            ...
            for i, (ox, oy) in enumerate(pcb_hole_offsets):
                world_pos = mat @ Vector((ox, oy, 0))
                bpy.ops.mesh.primitive_cylinder_add(
                    vertices=16,
                    radius=pcb_hole_diameter / 2,
                    depth=pcb_hole_depth,
                    location=world_pos
                )
        ...
        if seperate_plate_from_body:
            ...
            bpy.context.object.modifiers["Boolean"].operand_type = 'COLLECTION'
            bpy.context.object.modifiers["Boolean"].solver = 'MANIFOLD'
            bpy.context.object.modifiers["Boolean"].collection = bpy.data.collections["PCB_MOUNT_HOLES"]
            bpy.ops.object.modifier_apply(modifier="Boolean")
    ...
    for collection in ['AXIS', 'SWITCH_SUPPORT', 'SWITCH_HOLE', 'SWITCH_HOLE_LOWER', 'SWITCH_PROJECTION', 'SWITCH_PROJECTION_INNER', 'KEYCAP_PROJECTION_OUTER', 'KEYCAP_PROJECTION_INNER', 'AMEOBA_CUT']:
        for thing in bpy.data.collections[collection].objects:
            bpy.context.view_layer.objects.active = thing
            thing.select_set(True)
        with suppress_stdout(): bpy.ops.object.delete()
